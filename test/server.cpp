...
namespace Test 
{
	struct Server2 {
	  ...
	  TestAPI::TestServerConnector connector;
	...
	TEST_CASE_METHOD(Server2, "v2_batch") {
	  ...
	  REQUIRE(TestAPI::TestServerConnector::VerifyMethodResult(1, batchresponse[0]) == 7);
	  REQUIRE(TestAPI::TestServerConnector::VerifyMethodResult(2, batchresponse[1]) == 304);
	  TestAPI::TestServerConnector::VerifyMethodError(-32602, R"(missing named parameter "b")", 3, batchresponse[2]);
	  TestAPI::TestServerConnector::VerifyMethodError(-32600, R"(invalid request)", nullptr, batchresponse[3]);
